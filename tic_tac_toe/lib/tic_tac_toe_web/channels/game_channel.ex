defmodule TicTacToeWeb.GameChannel do
  ...
  def join("game:" <> id, _, socket) do
    game =
      case GameSupervisor.new_game(id) do
        {:ok, pid} -> pid
        {:error, {:already_started, pid}} -> pid
        {:error, other} -> IO.inspect(other)
      end
  ...
  def handle_in("select", %{"coord" => [x, y]}, socket) do
    ...
    if Game.my_turn?(game, socket.assigns.player) do
      ...
      {:reply, {:error, %{reason: "It isn't your truth"}}, socket}
    end
  end
  ...
  defp notify_turn(%{assigns: %{player: player, game_id: game_id}} = socket, _status) do
